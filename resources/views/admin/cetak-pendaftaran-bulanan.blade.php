...
<html>
<head>
    <title>Cetak Data Pendaftaran Magang Bulanan</title>
    <style>
        body {
...
<body>
    <div class="container">
        ...
        <div class="header">
            <h2>DATA PENDAFTARAN MAGANG BULANAN</h2>
            <p>Bulan: {{ $month }} / Tahun: {{ $year }}</p>
            <p>Tanggal Cetak: {{ date('d-m-Y H:i:s') }}</p>
        </div>

        @if(count($pendaftaranMagangs) > 0)
            @foreach($pendaftaranMagangs as $index => $pendaftaran)
                <h3>Pendaftaran #{{ $index + 1 }}</h3>
                <table>
                    <tr>
                        <th width="30%">Nama</th>
            ...
            @endforeach
        @else
            <p>Tidak ada data pendaftaran magang untuk bulan dan tahun yang dipilih.</p>
        @endif
