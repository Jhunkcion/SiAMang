...
<body>
    ...
    <div class="light-rays">
        <script>
        // Create light rays with device-aware count - avoid redeclaration
        const isMobileLR = window.innerWidth <= 768;
        const isSmallScreenLR = window.innerWidth <= 480;
        const rayCountLR = isSmallScreenLR ? 8 : (isMobileLR ? 12 : 20);

        for (let i = 0; i < rayCountLR; i++) {
    ...
    <div class="particles">
        <script>
        // Create particles with device-aware count - avoid redeclaration
        const isMobilePE = window.innerWidth <= 768;
        const isSmallScreenPE = window.innerWidth <= 480;
        const particleCountPE = isSmallScreenPE ? 20 : (isMobilePE ? 40 : 60);

        for (let i = 0; i < particleCountPE; i++) {
            ...
            let sizeMultiplier = isSmallScreenPE ? 0.7 : (isMobilePE ? 0.85 : 1);
            let size;
            if (randomType === 'white') {
            ...
            const durationMultiplier = isMobilePE ? 0.8 : 1;
            ...
            const delayMultiplier = isMobilePE ? 0.7 : 1;
    ...
    <div class="container d-flex justify-content-center align-items-center flex-grow-1 py-5">
        <div class="card card-login">
            ...
            <form method="POST" action="{{ route('password.update') }}" novalidate>
                ...
                <button type="submit" class="btn-login" aria-label="Reset Password">
