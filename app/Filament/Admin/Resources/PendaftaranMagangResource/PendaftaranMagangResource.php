...
class PendaftaranMagangResource extends Resource
{
    ...
    protected static ?int $selectedPeriodeId = null;
                                            ...
                                            ->schema([
                                                ...
                                                Forms\Components\Placeholder::make('durasi_magang')
                                                    ...
                                                    ->hidden(function (?PendaftaranMagang $record): bool {
                                                        return !$record || !$record->exists || !$record->tanggal_mulai || !$record->tanggal_selesai;
                                                    }),
                                                     
                                                Forms\Components\TextInput::make('created_at')
                                                    ->label('Tanggal Pendaftaran')
                                                    ->formatStateUsing(fn (?string $state) => $state ? Carbon::parse($state)->format('d M Y H:i') : '-')
                                                    ->disabled()
                                                    ...
                                                    ->options(function () {
                                                        return InternshipRequirement::orderBy('deadline', 'desc')
                                                            ...
                                                            ->mapWithKeys(function ($item) {
                                                                ...
                                                                return [$item->id => $statusIcon . $item->period . " ({$item->quota} kuota)"];
                                                            });
                                                    })
...
->filters([
            ...
            Action::make('print_monthly')
                ...
                ->action(function (array $data, $livewire) {
                    $url = route('pendaftaran.printMonthly', ['year' => $data['year'], 'month' => $data['month']]);
                    $livewire->redirect($url, true);
                }),
        ])
        ->groups([
                Group::make('status')
    ...
    {
        // Eager load user and anggota relationships to reduce queries
    ...
    public static function getPages(): array
    {
        return [
            ...
            // Removed create page route to disable creation of new Pendaftaran Magang
